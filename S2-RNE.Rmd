---
title: "Supplementary material S2: Biodiversity net effect and resilience"
date: '`r Sys.Date()`'
output:
...
We want to merge two functions to analyse the output of ecosystem after disturbance: (i) the biodiversity net effect (NE) and its partitionning in complementarity (CE) and selection (SE) effects, and (ii) the resilience function of the ecosystem. More particularly we computed $R[NE(X)]$, and we want to know if $R[NE(X)]=NE[R(X)]$. In order to do that we will proceed in three times. First we need to define all mathematic functions. Then we will use a simulation approach to test for the equality (easy way to reject it). Finally if the simulation did not rejected the equality we will try to prove it mathematically.

# Mathematic definitions

We first need to define the resilience function, the biodiversity net effect functions, and their composed functions for any variable $X$ from the ecosystem.

## Resilience

Resilience has been defined by us as:

\begin{equation}
  R(t) = \frac{X_T(t)}{X_C(t)}
  (\#eq:R)
\end{equation}

where $X_T(t)$ and $X_C(t)$ are the ecosystem metric values at time $t$ in the disturbed simulation and in the undisturbed control simulation.
...
\begin{equation}
  \begin{array}{c}
    R[NE(X)]=\frac{NE_T(X)}{NE_C(X)} \\
    R[NE(X)]=\frac{CE_T(X)}{NE_C(X)} + \frac{SE_T(X)}{NE_C(X)} \\
...
\begin{equation}
  \begin{array}{c}
    ...
    \Delta _R[R(X)]_{sp} = \frac{ \frac{{X_{sp}}_T(mixture)}{{X_{sp}}_C(mixture)} }{       \frac{{X_{sp}}_T(monoculture)}{{X_{sp}}_C(monoculture)} } - P_{sp} \\
    R(M_{sp})= \frac{{M_{sp}}_T}{{M_{sp}}_C} \\
...
Let's imagine a mixture ($C_T$) of three species $sp1$, $sp2$ and $sp3$ with an ecosystem variable $X$ evolving after a disturbance event, the corresponding three monoculture (${M_{sp1}}_T$, ${M_{sp2}}_T$, ${M_{sp3}}_T$) which encountered the same disturbance, and the corresponding control mixture ($C_C$) and three control monoculture (${M_{sp1}}_C$, ${M_{sp2}}_C$, ${M_{sp3}}_C$) which did not encoutered any disturbance.
...
\begin{equation}
  \begin{array}{c}
    \Delta _R[R(X)]_{sp} = \frac{ \frac{{X_{sp}}_T(mixture)}{{X_{sp}}_C(mixture)} }{ \frac{{X_{sp}}_T(monoculture)}{{X_{sp}}_C(monoculture)} } - P_{sp} \\
    \Delta _R[R(X)]_{sp} = 0 \\
    \frac{ \frac{{X_{sp}}_T(mixture)}{{X_{sp}}_C(mixture)} }{ \frac{{X_{sp}}_T(monoculture)}{{X_{sp}}_C(monoculture)} } = P_{sp} \\
    \frac{{X_{sp}}_T(mixture)}{{X_{sp}}_C(mixture)} = P_{sp} * \frac{{X_{sp}}_T(monoculture)}{{X_{sp}}_C(monoculture)}\\
    \frac{{X_{sp}}_T(mixture)}{{X_{sp}}_T(monoculture)}*\frac{1}{{X_{sp}}_C(mixture)} = P_{sp} * \frac{1}{{X_{sp}}_C(monoculture)}\\
    \frac{{X_{sp}}_T(mixture)}{{X_{sp}}_T(monoculture)} = P_{sp} * \frac{{X_{sp}}_C(mixture)}{{X_{sp}}_C(monoculture)}\\
    \frac{{X_{sp}}_T(mixture)}{{X_{sp}}_T(monoculture)} + P_{sp} - P_{sp} = P_{sp} * (\frac{{X_{sp}}_C(mixture)}{{X_{sp}}_C(monoculture)} + P_{sp} - P_{sp})\\
    {\Delta _R(X)_{sp}}_T + P_{sp} = P_{sp} * ({\Delta _R(X)_{sp}}_C + P_{sp}) \\
    {\Delta _R(X)_{sp}}_T + 1 = {\Delta _R(X)_{sp}}_C + P_{sp}, \forall P_{sp} \neq 0\\
    {\Delta _R(X)_{sp}}_T = {\Delta _R(X)_{sp}}_C + P_{sp} - 1 \\
...
\begin{equation}
  \begin{array}{c}
    \Delta _R[R(X)]_{sp} = \frac{ \frac{{X_{sp}}_T(mixture)}{{X_{sp}}_C(mixture)} }{ \frac{{X_{sp}}_T(monoculture)}{{X_{sp}}_C(monoculture)} } - P_{sp} \\
    \Delta _R[R(X)]_{sp} = a \\
    \frac{ \frac{{X_{sp}}_T(mixture)}{{X_{sp}}_C(mixture)} }{ \frac{{X_{sp}}_T(monoculture)}{{X_{sp}}_C(monoculture)} } = (P_{sp} + a) \\
    \frac{{X_{sp}}_T(mixture)}{{X_{sp}}_C(mixture)} = (P_{sp} + a) * \frac{{X_{sp}}_T(monoculture)}{{X_{sp}}_C(monoculture)}\\
    \frac{{X_{sp}}_T(mixture)}{{X_{sp}}_T(monoculture)}*\frac{1}{{X_{sp}}_C(mixture)} = (P_{sp} + a) * \frac{1}{{X_{sp}}_C(monoculture)}\\
    \frac{{X_{sp}}_T(mixture)}{{X_{sp}}_T(monoculture)} = (P_{sp} + a) * \frac{{X_{sp}}_C(mixture)}{{X_{sp}}_C(monoculture)}\\
    \frac{{X_{sp}}_T(mixture)}{{X_{sp}}_T(monoculture)} + P_{sp} - P_{sp} = (P_{sp} + a) * (\frac{{X_{sp}}_C(mixture)}{{X_{sp}}_C(monoculture)} + P_{sp} - P_{sp})\\
    {\Delta _R(X)_{sp}}_T + P_{sp} = (P_{sp} + a) * ({\Delta _R(X)_{sp}}_C + P_{sp}) \\
    {\Delta _R(X)_{sp}}_T = (P_{sp} + a)*{\Delta _R(X)_{sp}}_C + (P_{sp} + a)*P_{sp} - P_{sp} \\
    {\Delta _R(X)_{sp}}_T = (P_{sp} + a)*{\Delta _R(X)_{sp}}_C + P_{sp}*(P_{sp} + a - 1)\\
...
\begin{equation}
  M[R(X)] = \frac{{X_{sp}}_T(monoculture)}{{X_{sp}}_C(monoculture)} = R[M(X)]
  (\#eq:M)
\end{equation}
