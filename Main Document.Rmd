...
author: Sylvain Schmitt$^{*,1,2}$, Isabelle Maréchaux$^3$, Jerome Chave $^4$, Fabian Fischer$^4$, Camille  Piponiot$^5$, Stéphane Traissac$^6$, Bruno Hérault$^{7,8}$
output:
...
$^3$ AMAP, INRA, University of Montpellier, IRD, CIRAD, CNRS, , Montpellier, France
...
$^7$ Cirad, UMR EcoFoG (AgroParistech, CNRS, Inra, Université des Antilles, Université de la Guyane), Campus Agronomique, 97310 Kourou, French Guiana

$^8$ INPHB, Institut National Polytechnique Félix Houphouët-Boigny, BP 1093, Yamoussoukro, Ivory Coast
...
As species richness ($SR$) increased from 5 to 125 species, functional diversity varied from $0.76\pm0.18$ to $0.96\pm0.02$ and evenness ($FEve$) varied from $0.46\pm0.24$ to $0.64\pm0.02$. Species richness ($SR$) was significantly correlated to functional indices (one-way ANOVA $p-value < 0.001$ for both $FDiv$ and $FEve$) and model $AIC$ was higher without $SR$, consequently we kept only $FDiv$ and $FEve$ within linear regressions. Both functional diversity ($FDiv$) and functional evenness ($FEve$) were positively related with $I_R$, i.e. forest resilience increased with both functional diversity and functional evenness (Fig. \@ref(fig:gIr) and Table \@ref(tab:tgIr)). Similar results were obtained for all disturbance levels, with smaller p-values for higher disturbance intensity.

## Post-disturbance trajectories of biodiversity effect

The net effect of biodiversity on each ecosystem property was quantified and partitioned into selection and complementarity effects after normalizing, as explained in the Methods section. We found that the complementarity effect sharply increased immediately after disturbance, was the dominant effect of biodiversity during the first decades and then progressively vanished (Fig. \@ref(fig:gBE)). When increasing the disturbance intensity, the effect of complementarity increased further and dominated even longer. The selection effect, on the other hand, sharply decreased just after disturbance, and only gradually came to play a more important role, dominating the net biodiversity effect ca. a century after disturbance. Finally, at high disturbance intensity, the net effect of biodiversity on aboveground biomass was still lower than the control 600 years after disturbance (Fig. \@ref(fig:gBE)). We found similar results, but with an amplified signal, for basal area ($BA$) and stem abundance ($N$). Finally, forest growth primary productivity ($GPP$) recovered only within a few years with a dominant complementarity effect.
...
Ir %>% 
  ...
  kable(caption = "Effects of functional diversity on forest resilience. Results of the linear regression testing the effect of functional diversity indices (FDiv and FEve) on the resilience index ($I_R$) for forest structure and functioning and for each level of disturbance intensity ($disturb_{intensity}$). To meet the assumption of normality $I_R$ was log-transformed. Test of FDiv and FEve effects was done by linear regression with FDiv and FEve as independent variables. Values of FDiv and FEve are coefficients from linear regressions. Significant $p-values$ are shown in bold. ns, non-significant; . p < 0.1; * p < 0.05; ** p < 0.01; *** p < 0.001.",
        col.names = c("$I_R$", "Disturbance intensity",
