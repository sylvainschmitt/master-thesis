...
$\tau_{young}$, $\tau_{mature}$, and $\tau_{old}$ are species residence times in each leaf pools ($years$). The sum of residency time thus defined the leaf lifespan $LL = \tau_{young} + \tau_{mature} + \tau_{old}$ ($years$). $\tau_{young}$ is set to one month and $\tau_{mature}$ is set to a third of leaf lifespan $LL$. Belowground carbon allocation is not simulated inside TROLL.
